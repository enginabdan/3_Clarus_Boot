...
SELECT DISTINCT COUNT(product_id) OVER() order_num_of_bike
FROM (SELECT DISTINCT product_id FROM sales.order_items) A
...
from (select distinct category_id, brand_id from production.products) A
...
SELECT	*, LAG(order_date, 1) OVER (PARTITION BY staff_id ORDER BY Order_date, order_id) prev_ord_date
FROM	sales.orders
